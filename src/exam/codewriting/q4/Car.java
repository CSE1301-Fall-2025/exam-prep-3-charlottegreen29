...
public class Car {
	...
	*/
	public void drive ( double milesPerHour , double hours ) {
		...
		double galsFuelConsumed = miles/this.fuelEfficiency;
		if (galsFuelConsumed<=this.amountFuel){
			this.odometer+=miles;
			this.amountFuel-=galsFuelConsumed;
		}
		else{
			miles = this.fuelEfficiency*this.amountFuel;
			this.odometer+=miles;
			this.amountFuel=0;
		}
	}
	...
	*/
	public double addFuel ( double gallons ) {
		if (gallons<this.fuelTank){
			this.amountFuel+=gallons;
			return 0;
		}
		this.amountFuel = fuelTank;
		return gallons-this.fuelTank;
	}
